...
class JsonSupport extends upickle.AttributeTagged {
  ...
  implicit val bigDecimalRW: ReadWriter[BigDecimal] =
    readwriter[Double].bimap[BigDecimal](_.toDouble, double => BigDecimal(double.toString))
  ...
  /* Java Time DayOfWeek */
  implicit val dayOfWeekR: ReadWriter[DayOfWeek] =
    readwriter[String].bimap(_.name, v => DayOfWeek.valueOf(v.toUpperCase))
