...
object Impl {
  ...
  /** The following two methods handle the rare edge case of user being invoiced for multiple subscriptions in a single
    * invoice. Such invoices have a single price, however MMA design displays invoices grouped by product. This happens
    * in the following cases:
    *   - customer has been through a conversation with a CSR on the phone who would have explained that there would be
    *     a product transition in their next bill; or
    *   - it's a result of some kind of manual account for a special reason (e.g. GW agent)
    *
    * This scenario can be simulated/tested by
    *   1. find existing account in Zuora 2. create multiple subscriptions via 'create new subscription' button with
    *      same contract/acceptance date 3. bill run with acceptance date 4. post invoices 5. process payment
    */
