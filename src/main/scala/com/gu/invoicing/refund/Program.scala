...
object Program {
  def program(input: RefundInput): RefundOutput = {
    ...
    val itemsByInvoiceId      = getItemsByInvoice(subscriptionName)                            tap { itemsByInvoiceId       => s"$subscriptionName should have at least one invoice item"                   assert (itemsByInvoiceId.nonEmpty) }

    val (invoiceId, _, items) = decideRelevantInvoice(invoices, itemsByInvoiceId)              tap { case (_, invoice, _)   => s"$invoice should be at least posted and not negative"                       assert (invoice.Amount > 0.0 && invoice.Status == "Posted") }
